...
class PartsListCtrl(wx.ListCtrl):
	...
	def Populate(self):
		...
		items = list(self.partsdata.items())
	...
	def OnAdd(self, item):
		...
		if num >= PartsListCtrl.MAX_ARABICPARTS_NUM:
			txt = mtexts.txts['MaxArabicPartsNum']+str(PartsListCtrl.MAX_ARABICPARTS_NUM)+'!'
	...
	def load(self, parts):
		if parts != None:
			...
			for i in range(num):
				#Formula to text
				formula = mtexts.partstxts[parts[i][1][0]]+' + '+mtexts.partstxts[parts[i][1][1]]+' - '+mtexts.partstxts[parts[i][1][2]]
...
class ArabicPartsDlg(wx.Dialog):
	...
	def OnAdd(self, event):
		...
		formula = mtexts.partstxts[self.acb.GetCurrentSelection()]+' + '+mtexts.partstxts[self.bcb.GetCurrentSelection()]+' - '+mtexts.partstxts[self.ccb.GetCurrentSelection()]
		diurnal = ''
