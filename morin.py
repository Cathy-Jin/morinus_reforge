...
class MFrame(mrclasses.MrTopFrame):
	...
	def __init__(self, parent, id, title, opts):
		...
		(self.ID_New, self.ID_Data, self.ID_Load, self.ID_Save, self.ID_SaveAsBitmap, self.ID_Synastry, self.ID_FindTime,\
			self.ID_Ephemeris, self.ID_Close, self.ID_Exit) = list(range(100, 110))
		...
		self.ID_PrimaryDirs) = list(range(110,132))

		#Charts-menu
		(self.ID_Transits, self.ID_Revolutions, self.ID_SunTransits, self.ID_SecondaryDirs, self.ID_Elections,
		self.ID_SquareChart, self.ID_ProfectionsChart, self.ID_MundaneChart) = list(range(140, 148))
		...
		self.ID_Nodes, self.ID_Orbs, self.ID_PrimaryDirsOpt, self.ID_PrimaryKeys, self.ID_PDsInChartOpt, self.ID_PDsInChartOptZod, self.ID_PDsInChartOptMun, self.ID_LotOfFortune, self.ID_ArabicParts, self.ID_Syzygy, self.ID_FixStarsOpt, self.ID_ProfectionsOpt, self.ID_Languages, self.ID_AutoSaveOpts, self.ID_SaveOpts, self.ID_Reload) = list(range(150, 180))
		...
		self.ID_Housesystem11, self.ID_Housesystem12) = list(range(1050, 1062))
...
#				self.calc()##
	...
	def onSynastry(self, event):
		...
		if chrt != None:
			txt = self.horoscope.name+' - '+chrt.name+' '+mtexts.txts['Synastry']+' ('+str(chrt.time.origyear)+'.'+common.common.months[chrt.time.origmonth-1]+'.'+str(chrt.time.origday)+' '+str(chrt.time.hour)+':'+str(chrt.time.minute).zfill(2)+':'+str(chrt.time.second).zfill(2)+')'
