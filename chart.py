...
#		abovehorizon = self.planets.planets[planets.Planets.SUN].speculums[planets.Planet.PLACIDIAN].abovehorizon
	...
	def calcFixStarAspMatrix(self):
		...
		for i in range(num):
			ar = []
			nom = self.fixstars.data[i][fixstars.FixStars.NOMNAME]
			orb = self.options.fixstars.get(nom, 0.0)
			val1 = self.fixstars.data[i][fixstars.FixStars.LON] + orb
			val2 = self.fixstars.data[i][fixstars.FixStars.LON] - orb
